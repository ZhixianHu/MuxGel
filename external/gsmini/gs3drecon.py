...
# Modified and adapted from https://github.com/duyipai/gsmini
...
class Reconstruction3DStatic:
    def __init__(self, cpuorgpu="cpu", net_path=None, orgReset=False, orgImgPath=None):
        self.cpuorgpu = cpuorgpu
        device = torch.device(cpuorgpu)

        if net_path is None:
            net_path = Path(__file__).resolve().parent / "nnmini.pt"
        elif not os.path.isfile(net_path):
            print("Error opening ", net_path, " does not exist")
            return
        net = RGB2NormNetR15().float().to(device)
        if cpuorgpu == "cuda":
        ...
        if orgReset and orgImgPath is not None:
            self.orgImg = cv2.imread(orgImgPath)
            self.dm_zero = self.get_rawdepthmap(self.orgImg, mask_markers=False)
        else:
            bckPath = Path(__file__).resolve().parent / "simInit.png"
            self.orgImg = cv2.imread(str(bckPath))
            self.dm_zero = self.get_rawdepthmap(self.orgImg, mask_markers=False)

        
    def get_rawdepthmap(self, frame, mask_markers=False, cm=None):
        MARKER_INTERPOLATE_FLAG = mask_markers
        """ find contact region """
        # cm, cmindx = find_contact_mask(f1, f0)
        ###################################################################
    ...
    def get_depthmap(self, frame, mask_markers=False, cm=None):
        dm = self.get_rawdepthmap(frame, mask_markers, cm)
        """ remove initial zero depth """
        dm = dm - self.dm_zero
...
class Visualize3D:
    def __init__(self, n, m, save_path = None):
        import open3d
        self.n, self.m = n, m
        self.init_open3D()
        self.cnt = 212
    ...
    def init_open3D(self):
        ...
        #---------------Modify
        flat_x = np.ndarray.flatten(self.X)
        flat_y = np.ndarray.flatten(self.Y)
        cx = (self.n - 1) / 2.0
        cy = (self.m - 1) / 2.0
        rot_x = -(flat_y - cy) + cx
        rot_y = -(flat_x - cx) + cy
