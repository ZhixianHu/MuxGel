...
from src.imgProcess_di_absT import imgFusion
from src.imgProcess_di_absT import tactImgUpdate
from src.imgProcess_di_absT import tactBgObtain, tactChange
from src.imgProcess_di_absT import nonContactLightMapObtain
...
class simFusedImgDataset(Dataset):
    ...
    def apply_correlated_jitter_cv(self, tactBg, tact, rgb, nonContactLightMap):
        ...
        tactBg = self.adjust_image_cv(tactBg, tact_brightness, tact_con, tact_sat, tact_hue)
        tact = self.adjust_image_cv(tact, tact_brightness, tact_con, tact_sat, tact_hue)
        rgb = self.adjust_image_cv(rgb, vis_brightness, vis_con, vis_sat, vis_hue)
        nonContactLightMap = self.adjust_image_cv(nonContactLightMap, vis_brightness, vis_con, vis_sat, vis_hue)
        return tactBg, tact, rgb, nonContactLightMap
    ...
    def __getitem__(self, idx):
        ...
        # color jittering
        tactBg, tact, rgb_img, nonContactLightMap = self.apply_correlated_jitter_cv(tactBg, tact, rgb_img, nonContactLightMap)
        

        fused_img, refImg, rgb_processed, rgb, ckMsk = imgFusion(rgb_img, contact_mask, background_mask, tact, 
                                                                       tactBg, nonContactLightMap, cell_num)

        to_tensor = T.ToTensor()
            
        sample = {
            "fused_img": to_tensor(fused_img),
            "ref_img": to_tensor(refImg),
            "rgb_processed": to_tensor(rgb_processed),
            "rgb": to_tensor(rgb),
            # "tact_img": torch.from_numpy(tact_img).permute(2, 0, 1).contiguous(),
            # "tact_shadow_img": torch.from_numpy(tact_shadow_img).permute(2, 0, 1).contiguous(),
            "tact": to_tensor(tact),
...
def train(args):
    ...
    #         T.Resize((args.downsample_h, args.downsample_w)),
    #         T.ToTensor()])
    # else:
    #     transform = T.Compose([
    #         T.ToPILImage(),
    ...
    # if len(val_full_ds) > val_limit_samples:
    #     val_g_cpu = torch.Generator()
    #     val_g_cpu.manual_seed(args.seed)
    #     indices = torch.randperm(len(val_full_ds), generator=val_g_cpu)[:val_limit_samples]
    #     val_subset_ds = Subset(val_full_ds, indices)
    #     print(f"Subsampling Validation Set: {len(val_full_ds)} -> {len(val_subset_ds)} images (Fixed Subset)")
    # else:
    #     val_subset_ds = val_full_ds
    #     print(f"Validation Set is small ({len(val_full_ds)}), using full set.")
    ...
    criterion = DecouplingLoss(w_tact=args.w_tact, w_vis=args.w_vis)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for batch in pbar:
            ...
            rgb_processed = batch["rgb_processed"].to(device)
            tact = batch["tact"].to(device)
            ck_msk = batch["ck_msk"].to(device)

            input_tensor = torch.cat([fused_img, ref_img], dim=1)

            with torch.cuda.amp.autocast():
                tact_pred, vis_pred = model(input_tensor)

                loss, loss_dict = criterion(tact_pred, tact, vis_pred, rgb_processed, mask=ck_msk)
            ...
            if run is not None and (pbar.n % args.log_interval == 0):
                with torch.no_grad():
                    ...
                    grid = np.concatenate([
                        ...
                        to_u8_hwc(tact[b0]),
                        to_u8_hwc(rgb_processed[b0]),
                        to_u8_hwc(tact_pred[b0]),
                        to_u8_hwc(vis_pred[b0]),
                    ], axis=1)
                    wandb.log({"preview": wandb.Image(grid, caption="fused | ref | tact | rgb | tact_pred | vis_pred")}, step=global_step)
        ...
        with torch.no_grad():
            for batch in val_loader:
                fused_img = batch["fused_img"].to(device)
                ref_img = batch["ref_img"].to(device).float()
                rgb_processed = batch["rgb_processed"].to(device)
                tact = batch["tact"].to(device)
                ...
                input_tensor = torch.cat([fused_img, ref_img], dim=1)
                tact_pred, vis_pred = model(input_tensor)
                evaluator.update(tact_pred, tact, vis_pred, rgb_processed)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--w_tact', type=float, default=1.0, help='Weight for tactile loss')
    parser.add_argument('--w_vis', type=float, default=0.5, help='Weight for vision loss')
    ...
    parser.add_argument('--run_name', type=str, default='sim_di_abst_run')
