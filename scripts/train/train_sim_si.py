...
from src.imgProcess_si import imgFusion
from src.imgProcess_si import imgFusionPredefined
...
class simFusedImgDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        fused_img, rgb_processed, rgb, ckMsk = imgFusion(rgb_img, contact_mask, background_mask, tact,
                                cell_num)
...
class DualResNet34UNet(nn.Module):
    def __init__(self, n_channels=3, n_classes_tact=3, n_classes_vis=3):
...
def train(args):
    ...
    model = DualResNet34UNet(n_channels=3, n_classes_tact=3, n_classes_vis=3).to(device)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for batch in pbar:
            ...
            with torch.cuda.amp.autocast():
                tact_pred, vis_pred = model(fused_img)
            ...
            if run is not None and (pbar.n % args.log_interval == 0):
                with torch.no_grad():
                    ...
                    wandb.log({"preview": wandb.Image(grid, caption="fused | tact | rgb | tact_pred | vis_pred")}, step=global_step)
        ...
        with torch.no_grad():
            for batch in val_loader:
                ...
                tact_pred, vis_pred = model(fused_img)
                # tact_pred = torch.sigmoid(tact_pred)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--run_name', type=str, default='sim_si_run')
